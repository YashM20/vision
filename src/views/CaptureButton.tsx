...
const START_RECORDING_DELAY = 0 // previously 200
...
interface Props extends ViewProps {
  ...
  photoModeEnabled: boolean // New prop to control photo mode
}
...
const _CaptureButton: React.FC<Props> = ({
  ...
  const onHandlerStateChanged = useCallback(
    async ({ nativeEvent: event }: TapGestureHandlerStateChangeEvent) => {
      ...
      switch (event.state) {
        case State.BEGAN: {
          ...
          if (photoModeEnabled) {
            // If photo mode is enabled, we don't want to record videos, so we just take a photo and return
            await takePhoto()
          }
          ...
          if (isRecording.current) {
            ...
            console.log('Recording already started. Stopping recording...');
            ...
            console.log('Recording not started yet. Ignoring stopRecording() call.')
    ...
    [isPressingButton, recordingProgress, setIsPressingButton, startRecording, stopRecording, takePhoto, photoModeEnabled],
  )
  //#endregion
  //#region Pan handler
